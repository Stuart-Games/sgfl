...
- **`sgfl init`** — scaffold a new project (creates `src/{client,server,shared,shared/Util}`, `assets.json`, `default.project.json`, `.env`, `.gitignore`, runs `rokit init/add lune/add rojo/install`).
...
| `PLACE_ID` | `.env` or `.env.<env>` | start, save | numeric place id |
| `UNIVERSE_ID` | `.env` or `.env.<env>` | start, publish | numeric experience id |
