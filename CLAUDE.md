...
- **`sgfl init`** — scaffold a new project (same layout as before: `src/{Shared,...}` with `Shared` wrapped under ReplicatedStorage, `assets.json`, `default.project.json`, `.env` with prompted `PLACE_ID`/`UNIVERSE_ID`, `.gitignore`, `rokit init/add rojo/install` — **no Lune**). No seed asset files are written: entry files appear on the first `sgfl save`, and a publish with a missing entry file simply keeps the base content for that entry.
...
- Adding a sidecar property? Add `(class, prop): typeId` to `ALLOWLIST` in [sidecar.py](src/sgfl/sidecar.py) (and `ANCHORS` if script-readable). Properties serialized but not enumerated by ReflectionService (e.g. `Lighting.Technology`) can only be found by round-trip testing — the reflection sweep cannot see them.
